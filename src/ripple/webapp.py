...
h1{font-size:3rem;letter-spacing:-.055em;margin:4px 0 5px}h2{letter-spacing:-.025em}.sub{font-size:1.12rem;max-width:760px;color:#596273;margin:0}.promise{font-weight:850;color:#252b36;margin-top:10px}
...
.turn{display:grid;grid-template-columns:76px 1fr;gap:12px;align-items:start;margin:11px 0}.who{font-size:.78rem;font-weight:850;letter-spacing:.02em;color:#555f70;padding-top:12px}.bubble{border-radius:15px;padding:13px 15px;line-height:1.48}.user-bubble{background:#eef2f8}.agent-bubble{background:#f7f8fa;border:1px solid #e1e5eb}.agent-bubble b{display:block;margin-bottom:3px}
textarea{width:100%;min-height:76px;resize:vertical;font:inherit;padding:0;border:0;outline:0;background:transparent;color:#202631}
...
.proof{font-weight:800}.ok{color:#17643b}.warn{color:#84520d}.muted{color:#697286}.micro{font-size:.82rem;line-height:1.45;color:#71798a}.hidden{display:none}.impact{border-left:3px solid #9ca5b4;padding:6px 0 6px 12px;margin:10px 0}
...
.runtime-note{border-left:3px solid #535c6d;padding:10px 12px;background:#f8f9fb;border-radius:0 12px 12px 0;margin-top:14px;font-size:.84rem;color:#596273}
...
@media(max-width:760px){h1{font-size:2.35rem}.wide,.side{grid-column:span 12}.metric-grid{grid-template-columns:1fr}.turn{grid-template-columns:1fr;gap:3px}.who{padding-top:0}}
</style>
</head>
<body><main>
<section class="hero"><div class="eyebrow">Amazon Developer Hackathon · rules-permitted simulated Alexa+ experience</div><h1>Ripple</h1><p class="sub">Tell Alexa one thing that changed. Ripple finds what breaks downstream, quantifies what is at risk, and asks for one exact approval before bounded repairs execute.</p><p class="promise">Repair the cascade without opening five apps/sites.</p></section>
<div class="grid">
<section class="card wide"><div class="eyebrow">Customer → agent</div><div class="turn"><div class="who">You</div><div class="bubble user-bubble"><textarea id="utterance" aria-label="Message to Alexa+">Our flight home was cancelled. We'll land tomorrow at 18:00.</textarea></div></div><button class="primary" onclick="propose()">Send to Alexa+</button><button onclick="resetDemo()">Reset</button><div class="runtime-note"><b>Evidence boundary:</b> this simulated Alexa+ UI runs the same Ripple planner, policy and receipt engine exposed by the public MCP service. The authenticated public MCP transport is verified separately by the remote smoke gate; this page does not pretend to be an official Alexa+ client.</div></section>
...
async function propose(){
  try{
    ...
    const j=await post('/api/propose',{utterance});
    ...
    document.getElementById('proposal').innerHTML=`<div class="eyebrow">Customer → agent → decision</div><div class="turn"><div class="who">You</div><div class="bubble user-bubble">${esc(utterance)}</div></div><div class="turn"><div class="who">Alexa+</div><div class="bubble agent-bubble"><b>Ripple found the downstream cascade.</b>“${esc(c.voice_summary)}”</div></div><h2>${esc(c.headline)}</h2><div class="money-line">${esc(c.money_summary)}</div><div class="metric-grid">${metrics}</div><h3>Top consequences</h3>${impacts}${more}<p class="muted">${esc(scope)}</p><p class="proof ok">✓ External writes before approval: ${j.writes_before_approval}</p><button class="primary" aria-label="${esc(c.decision.voice_prompt)}" onclick="approve()">${esc(c.decision.label)}</button>${technicalEvidence(p,d)}`;
...
async function approve(){try{if(!currentApproval)throw new Error('No captured approval snapshot');const j=await post('/api/approve',currentApproval);document.getElementById('execution').classList.remove('hidden');document.getElementById('execution').innerHTML=`<div class="eyebrow">Customer-approved execution</div><div class="turn"><div class="who">Alexa+</div><div class="bubble agent-bubble"><b>Done.</b>${j.receipt_count} approved repair actions executed. Here are the authoritative receipts.</div></div><h2>Authoritative receipts</h2><p class="proof ok">✓ Plan status: ${esc(j.plan_status)} · unique external writes: ${j.external_write_count}</p>${receiptTable(j.receipts)}<button onclick="replay()">Replay exact approved plan</button><div id="replayProof"></div>`}catch(e){alert(e.message)}}
